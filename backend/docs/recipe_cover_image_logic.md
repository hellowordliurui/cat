...
2. **生成「生图提示语」**（`_cover_prompt(recipe)`）  
   - **硬性约束**：  
     - 画面里**只能是「最终成品」**：步骤全部做完、已经装盘/盛碗、可以上桌的那一道菜。  
     - **不能**出现：生食材、制作过程、手、锅具等。  
   - **成品形态**（`_final_shape_from_steps(steps)`）：  
     - 根据**步骤文案**推断这道菜做完后长什么样，例如：  
       - 步骤里出现「脱模」「模具」「慕斯」→ 成品是「脱模后的慕斯/布丁，小杯或半球在盘上」；  
       - 「塔」「叠」「层」→ 「分层塔/叠层在盘上」；  
       - 「浓汤」「羹」「汤」→ 「一碗成品浓汤/羹」；  
       - 「装盘」「盛出」「碗」→ 「已装盘或在小碗里的成品」；  
       - 「冷藏」「定型」→ 「冷藏定型后的成品」；  
   ...
   - **菜名 + 食材**：  
     - 明确写「一道菜：<菜名>」；  
     - 成品形态用上面推断的英文描述；  
     - 前 4 种食材写成「成品中可见/由这些食材制成」，避免被画成生料堆。  
   - 最后再补一句：猫用、有食欲、柔光、俯拍或小角度、干净盘/碗。
...
总结：**生成逻辑 = 用「菜名 + 品类 + 步骤推断的成品形态 + 主要食材」拼成一段「只画最终成品」的英文提示 → 豆包生图 → 存到 Storage 或本地 → 得到 URL**。
...
新食谱入库 (POST /api/recipes)
  ...
  → 若 recipe.imageURL 为空：
       → _cover_prompt(recipe) 用「菜名+步骤形态+食材」生成提示
       → 豆包文生图
